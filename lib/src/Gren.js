...
class Gren {
    ...
    _getTagDates(tags) {
        return tags.map(async tag => {
            ...
            const tagDate = new Date(Date.parse(committer.date) + 60 * 1000).toISOString();
            return {
                id: tag.releaseId,
                name: tag.tag.name,
                date: tagDate
    ...
    async _getIssueBlocks(releaseRanges) {
        ...
        const release = releaseRanges
            .map(range => {
                const filteredIssues = Array.from(issues).filter(this._filterIssue.bind(this));
                const body = (!range[0].body || this.options.override) && this._groupByLabel(filteredIssues);// this._groupBy(issues);
                return {
                    id: range[0].id,
                    release: range[0].name,
                    name: this.options.prefix + range[0].name,
